...
* Кликним 4 раза по углам корта на изображении.
* Координаты сохранятся в `pts_img.json`.

### Пример кадра с глубиной

также можно визуально проверить, как выглядит карта глубины (высоты над кортом) из `rgbd_tensor.pt`.
Вот пример:
...
можно было получить наверное результат получше,видимо неудачно выбранный видос, или если было больше времени наверное было бы лучше так, как начал на выходных, так как всю неделю работал...
